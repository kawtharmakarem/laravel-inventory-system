...
class CollectController extends Controller
{
  public function index()
  {
    ...
    $account_search = Account::select('name', 'account_type', 'account_number')->where(['com_code' => $com_code, 'is_parent' => 0])->orderby('id', 'DESC')->get();
    ...
    return view('admin.collect_transactions.index', ['data' => $data, 'checkExistsOpenShift' => $checkExistsOpenShift, 'accounts' => $accounts, 'mov_type' => $mov_type, 'account_search' => $account_search, 'treasuries' => $treasuries, 'admins' => $admins]);
  }
    ...
    if ($request->ajax()) {
      ...
      if (!empty($AccountData)) {
        if ($AccountData['account_type'] == 2) {
          ...
          return view('admin.collect_transactions.get_account_balance', ['the_final_balance' => $the_final_balance]);
